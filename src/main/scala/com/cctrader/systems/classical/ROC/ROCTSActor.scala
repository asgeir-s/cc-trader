 ...
 *
 */
class ROCTSActor(marketDataSetIn: MarketDataSet, signalWriterIn: Signaler, settingPathIn: String) extends {
...
object ROCTSActor {
  def props(trainingMarketDataSet: MarketDataSet, signalWriterIn: Signaler, tsSettingPath: String): Props =
    Props(new ROCTSActor(trainingMarketDataSet, signalWriterIn, tsSettingPath))
}
