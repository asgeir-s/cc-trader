 ...
 *
 */
class OppositeWilliamRTSActor(marketDataSetIn: MarketDataSet, signalWriterIn: Signaler, settingPathIn: String) extends {
...
object OppositeWilliamRTSActor {
  def props(trainingMarketDataSet: MarketDataSet, signalWriterIn: Signaler, tsSettingPath: String): Props =
    Props(new OppositeWilliamRTSActor(trainingMarketDataSet, signalWriterIn, tsSettingPath))
}
